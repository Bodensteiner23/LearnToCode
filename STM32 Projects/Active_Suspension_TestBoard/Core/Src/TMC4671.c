...
/* ================================ Typedefs ================================= */
//Variable for SPI (added by Daniel Jäger)
static SPI_HandleTypeDef* tmc_hspi;
/* ================================ Parameters ================================= */

/* ================================ Variables =============================== */

/* ================================ Function Definitions =================== */
//SPI-Init to hand over pointers (added by Daniel Jaeger)
void tmc_initSpi(SPI_HandleTypeDef* hspi) {
	tmc_hspi = hspi;
	//ToDo: activate if Hardware NSS is used
	//HAL_GPIO_WritePin(SPI_NSS_GPIO_Port, SPI_NSS_Pin, DISABLE);
}
// => SPI wrapper (changed from extern to own function by Daniel Jaeger)
uint8_t tmc4671_readwriteByte(uint8_t motor, uint8_t data, uint8_t lastTransfer) {
	uint8_t txData = data;
	uint8_t rxData = 0x00;

	HAL_GPIO_WritePin(SPI3_CS_TMC_GPIO_Port, SPI3_CS_TMC_Pin, DISABLE);
	HAL_SPI_TransmitReceive(tmc_hspi, &txData, &rxData, 1, 5);
	if(lastTransfer) {
		HAL_GPIO_WritePin(SPI3_CS_TMC_GPIO_Port, SPI3_CS_TMC_Pin, ENABLE);
	}
